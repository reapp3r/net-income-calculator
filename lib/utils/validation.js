 ...
 *
 * @example
 * validateCurrencyCode('EUR'); // ✅ Returns { valid: true, code: 'EUR', error: null }
 * validateCurrencyCode('€'); // ❌ Returns { valid: false, code: null, error: '...' }
 */
function validateCurrencyCode(code) {
  if (!code || typeof code !== 'string') {
    return { valid: false, code: null, error: `Invalid currency code: ${code}` };
  }
  ...
  if (!/^[A-Z]{3}$/.test(normalized)) {
    return {
      valid: false,
      code: null,
      error: `Invalid currency code format: ${code}. Must be ISO 4217 (e.g., EUR, GBP, USD)`
    };
  }
...
function validateIncomeType(
  incomeType,
  validTypes = ['employment', 'freelance', 'dividend']
) {
  if (!incomeType) {
  ...
  if (!validTypes.includes(type)) {
    return {
      valid: false,
      type: null,
      error: `Invalid income type: ${incomeType} (must be one of: ${validTypes.join(', ')})`,
    };
  }
