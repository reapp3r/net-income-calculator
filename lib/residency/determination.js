...
class ResidencyDetermination {
  ...
  determineResidency(data, manualOverrides) {
    ...
    for (const year of years) {
      ...
      if (splitYear) {
        ...
        const periods = [];
        for (const period of splitYear) {
          const residency = this.determineForPeriod(period, data);
  ...
  detectSplitYear(year, locationData) {
    ...
    const filteredRelocations = relocations.filter(reloc => {
      // Must have at least 30 days between relocations
      const daysDiff = (new Date(reloc.date) - new Date(reloc.previousDate)) / (1000 * 60 * 24);
      return daysDiff >= 30;
    });
    ...
    let relocationDate = new Date(permanentRelocation.date);
    while (relocationDate.getFullYear() < year) {
      relocationDate = new Date(relocationDate.getFullYear() + 1, relocationDate.getMonth(), relocationDate.getDate());
    }
  ...
  applyOECDTieBreaker(year, residentCountries, data) {
    ...
    const vitalInterests = residentCountries.map(r => ({
      country: r.country,
    ...
    const daysByCountry = residentCountries.map(r => ({
      country: r.country,
  ...
  countDaysInCountry(year, countryCode, locationData) {
    ...
    const countryLocations = (locationData || []).filter(loc =>
      loc.Country === countryCode && loc.LocationType === 'Residence'
    );
