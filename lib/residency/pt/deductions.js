...
function getPersonalDeductionsForMonth(year, month, personalDeductionsData) {
  const deduction = personalDeductionsData.find(d => 
...
function applyAllDeductions(taxAmount, deductionBreakdown, year, deductionsData) {
  ...
  if (deductionBreakdown.specificDeduction) {
    total.specificDeduction = Math.min(deductionBreakdown.specificDeduction, taxAmount);
    total.finalTax -= total.specificDeduction;
  }

  // Apply health expenses
  if (deductionBreakdown.healthExpenses) {
    total.healthExpenses = Math.min(deductionBreakdown.healthExpenses, taxAmount - total.finalTax);
    total.finalTax -= total.healthExpenses;
  }

  // Apply education expenses
  if (deductionBreakdown.educationExpenses) {
    total.educationExpenses = Math.min(deductionBreakdown.educationExpenses, taxAmount - total.finalTax);
  ...
  if (deductionBreakdown.ivaDeduction) {
    total.ivaDeduction = Math.min(deductionBreakdown.ivaDeduction, taxAmount - total.finalTax);
  ...
  if (deductionBreakdown.ivaBooksCulture) {
    ...
    if (rate > 0) {
      ...
      total.ivaBooksCulture = Math.min(ivaBooksAmount, taxAmount - total.finalTax);
  ...
  if (deductionBreakdown.otherDeduction) {
    total.otherDeduction = Math.min(deductionBreakdown.otherDeduction, taxAmount - total.finalTax);
