...
class PortugalResidency extends TaxResidency {
  ...
  calculateTax(grossIncome, incomeType, options = {}) {
    const { year, specialRegime, sourceCountry = 'PT', freelanceType, expenses, aggregate = false } = options;
    ...
    switch (incomeType) {
      case 'employment':
        return this.calculateEmploymentTax(grossIncome, specialRegimeStatus, isPortugueseSource, year);

      case 'freelance':
        return this.calculateFreelanceTax(grossIncome, specialRegimeStatus, isPortugueseSource, freelanceType, expenses, year);
  ...
  isEUEEA(countryCode) {
    ...
    const eueeaCountries = [
      // EU-27
      'PT', 'ES', 'FR', 'DE', 'IT', 'NL', 'BE', 'LU', 'AT', 'GR', 
      'IE', 'FI', 'SE', 'DK', 'PL', 'CZ', 'HU', 'SK', 'SI', 'EE',
