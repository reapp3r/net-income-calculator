 ...
 */
function calculateForeignTaxCredit(totalIncome, foreignIncome, totalTax, foreignTaxPaid) {
  ...
  return {
    ...
    note: unusedCredit > 0 ? 'Unused credit lost (Portugal FTC limitation)' : 'Full credit applied',
  };
}

function getWithholdingRate(year, sourceCountry, incomeType, foreignTaxCreditData) {
  const ftcData = getTemporalMatch(foreignTaxCreditData, year, 'SourceCountry', sourceCountry);
