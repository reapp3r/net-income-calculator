...
class GBResidency extends TaxResidency {
  ...
  testResidency(year, data) {
    const hasGBIncome = data.incomeRecords && data.incomeRecords.some(r =>
  ...
  calculateTax(grossIncome, incomeType, options = {}) {
    ...
    const { year, specialRegime, sourceCountry = 'GB', freelanceType, expenses, aggregate = false } = options;

    // UK has no special regimes (yet)
    const specialRegimeStatus = specialRegime ? { active: false } : { active: false };

    const isUKSource = sourceCountry === 'GB';

    switch (incomeType) {
      case 'employment':
        return this.calculateEmploymentTax(grossIncome, specialRegimeStatus, isUKSource, year);
      
      case 'freelance':
        return this.calculateFreelanceTax(grossIncome, specialRegimeStatus, isUKSource, freelanceType, expenses, year);
      
      case 'dividend':
        return this.calculateDividendTax(grossIncome, specialRegimeStatus, sourceCountry, aggregate, year);
  ...
  _getShiftedTaxBrackets(year, type) {
    ...
    const brackets = this.referenceData.taxBrackets.filter(b => b.IncomeType === type);
    
    if (type !== 'income') {
      return brackets;
    }

    const stdPADeduction = this.referenceData.deductions.find(d => d.Year == year && d.Type === 'PersonalAllowance');
  ...
  calculateEmploymentTax(grossIncome, specialRegimeStatus, isUKSource, year) {
    ...
    const taxAmount = calculateUKProgressiveTax(taxableIncome, year, incomeBrackets);
  ...
  calculateFreelanceTax(grossIncome, specialRegimeStatus, isUKSource, freelanceType, expenses = 0, year) {
    // Tax calculation is done entirely in GBP
    const incomeGBP = grossIncome;
    ...
    // Calculate income tax
    const incomeBrackets = this._getShiftedTaxBrackets(year, 'income');
    const taxAmount = calculateUKProgressiveTax(afterAllowance, year, incomeBrackets);
  ...
  calculateDividendTax(grossIncome, specialRegimeStatus, sourceCountry, aggregate, year) {
    ...
    const dividendBands = this.referenceData.taxBrackets.filter(band => band.IncomeType === 'dividend');
    const taxAmount = calculateUKProgressiveTax(taxableIncome, year, dividendBands);
