...
class GBResidency extends TaxResidency {
  ...
  getTaxBrackets(year) {
    if (!this.referenceData || !this.referenceData.taxBrackets) {
      return [];
    }
  ...
  getSocialSecurityRates(year) {
    if (!this.referenceData || !this.referenceData.socialSecurity) {
      return [];
    }
    return this.referenceData.socialSecurity.filter((s) => s.Year === year);
  }

  getSolidarityTaxConfig(_year) {
    // UK has no solidarity tax
    return null;
  }

  getSpecialRegimes(year) {
    if (!this.referenceData || !this.referenceData.specialRegimes) {
      return [];
    }
  ...
  _getShiftedTaxBrackets(year, type) {
    ...
    return brackets
      ...
      .filter((b) => {
        ...
        if (b.MaxIncome !== null && b.MaxIncome <= b.MinIncome) {
          return false;
        }
        return true;
      });
  ...
  calculateWithholdingForIncome(_grossIncome, _incomeType, _sourceCountry, _year) {
