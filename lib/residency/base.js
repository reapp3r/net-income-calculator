...
class TaxResidency {
  ...
  testResidency(year, data) {
    if (data.incomeRecords) {
      ...
      const hasIncome = data.incomeRecords.some((r) => r.Year === year || r.year === year);
   ...
   * Must be implemented by each country's residency class.
   *
   * @param {number} _grossIncome - Gross income amount
   * @param {string} _incomeType - Type of income (employment, freelance, dividend)
   * @param {Object} _options - Calculation options (year, sourceCountry, etc.)
   ...
   *   - isExempt: boolean
   */
  calculateTax(_grossIncome, _incomeType, _options = {}) {
   ...
   * Generic implementation - can be overridden by countries
   *
   * @param {Object} _regime - Regime configuration
   * @param {Object} _options - Conditions to check
   * @returns {boolean} True if regime applies
   */
  isRegimeApplicable(_regime, _options = {}) {
   ...
   *
   * @param {number} _grossIncome - Gross income amount
   * @param {string} _incomeType - Type of income
   * @param {string} _sourceCountry - Source country code
   * @returns {number} Withholding tax amount (default 0)
   */
  calculateWithholdingForIncome(_grossIncome, _incomeType, _sourceCountry) {
   ...
   */
  calculateStandardTaxOnIncome(_grossIncome, specialRegimeStatus, year) {
    // Default: Implement basic calculation or defer to calculateTax()
    return this.calculateTax(_grossIncome, 'employment', {
