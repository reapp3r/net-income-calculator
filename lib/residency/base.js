...
class TaxResidency {
  constructor(referenceData) {
    if (this.constructor === TaxResidency) {
      throw new Error(
        'TaxResidency is abstract and cannot be instantiated directly'
      );
    }
   ...
   *   - isExempt: boolean
   */
  calculateTax(grossIncome, incomeType, _options = {}) {
  ...
  testResidency(year, data) {
    if (data.incomeRecords) {
      ...
      const hasIncome = data.incomeRecords.some(
        r => r.Year == year || r.year == year
      );
