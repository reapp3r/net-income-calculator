...
function calculateNetIncome(incomeRecords, referenceData, options = {}) {
  ...
  // Supports both legacy (single country) and multi-country modes
  ...
  // 3. Determine tax residency per month using ResidencyCountry field
  // 4. Handle different fiscal years (calendar for PT, April-April for UK)
  ...
  // Handle both single-country (legacy) and multi-country reference data
  const isMultiCountry = typeof referenceData === 'object' && referenceData.PT && referenceData.GB;
  
  const parsedRecords = incomeRecords.map(record => {
    const residencyCountry = (record.ResidencyCountry || record.TaxResidency || options.defaultCountry || 'PT').toUpperCase();
    const residencyRefData = isMultiCountry ? referenceData[residencyCountry] : referenceData;
    const residency = getResidency(residencyCountry, residencyRefData);
    
    return parseIncomeRecord(record, { 
      residency, 
      defaultCountry: options.defaultCountry,
      simulationParameters: residencyRefData ? residencyRefData.simulationParameters : {}
    });
  });

  const byYear = {};
  for (const record of parsedRecords) {
    const residencyCountry = record.residencyCountry;
    const residencyRefData = isMultiCountry ? referenceData[residencyCountry] : referenceData;
    const residency = getResidency(residencyCountry, residencyRefData);
  ...
  for (const key of Object.keys(byYear)) {
    ...
    const residencyRefData = isMultiCountry ? referenceData[residencyCountry] : referenceData;
    const yearResults = processYear(fiscalYear, residencyCountry, records, currentYear, residencyRefData);
...
function processYear(year, residencyCountry, yearRecords, currentYear, referenceData) {
  ...
  for (const record of yearRecords) {
    // Convert to local currency for aggregation
    const localAmount = residency.convertToLocalCurrency(
      record.grossIncome, 
    ...
    aggregates[record.incomeType].bySource[key] += localAmount;
    aggregates[record.incomeType].total += localAmount;
  }

  // Get appropriate deduction functions based on residency
  const deductionsModule = residency.getDeductionsModule ? residency.getDeductionsModule() : require('./residency/pt/deductions');

  const monthly = yearRecords.map(record => {
    // Convert to local currency for tax calculations
    const localCurrency = residency.getDefaultCurrency();
    const localAmount = residency.convertToLocalCurrency(
      record.grossIncome, 
      record.sourceCurrency, 
      record.exchangeRate
    ...
    switch (record.incomeType) {
      case 'employment':
        taxResult = residency.calculateEmploymentTax(localAmount, regimeStat, isResidencySource, year);
        break;
    ...
    const withholding = residency.calculateWithholdingForIncome(localAmount, record.incomeType, record.sourceCountry, year);
    const socialSec = residency.getSocialSecurityAmount(localAmount, record.incomeType, record.freelanceType, year);

    // Get deduction functions from residency
    const deductionsModule = residency.getDeductionsModule();
    const deductionBreakdown = deductionsModule.getPersonalDeductionsForMonth ? 
      deductionsModule.getPersonalDeductionsForMonth(year, record.month, referenceData.personalDeductions) : 
      { totalDeductions: 0 };
    
    // Get additional tax components (e.g., solidarity tax) from residency
    const additionalTaxes = residency.getAdditionalTaxes ? residency.getAdditionalTaxes(taxResult, record, year) : {};
    const totalTaxBeforeDeductions = taxResult.taxAmount + (additionalTaxes.amount || 0);
    
    const deductionResult = deductionsModule.applyAllDeductions ? 
      deductionsModule.applyAllDeductions(totalTaxBeforeDeductions, deductionBreakdown, year, referenceData.deductions) :
      { finalTax: totalTaxBeforeDeductions, totalDeductions: 0 };
    ...
    const monthlyRecord = {
      ...
      Month: record.month,
      GrossIncome: localAmount.toFixed(2),
      ...
      NetIncome: (localAmount - withholding - deductionResult.finalTax - socialSec).toFixed(2)
    };

    // Add country-specific tax fields (e.g., solidarity tax)
    if (additionalTaxes && Object.keys(additionalTaxes).length > 0) {
      const formattedTaxes = residency.formatAdditionalTaxesForOutput(additionalTaxes);
      Object.assign(monthlyRecord, formattedTaxes);
    }
  ...
  for (const [incomeType, data] of Object.entries(aggregates)) {
    ...
    // Get additional tax components for this income type
    const additionalTaxes = residency.getAdditionalTaxes ? residency.getAdditionalTaxes(taxResult, {incomeType, data}, year) : {};
    const additionalTaxAmount = additionalTaxes.amount || 0;
    ...
    const typeRecord = {
      ...
      NetIncome: (data.total - taxResult.taxAmount - additionalTaxAmount - (taxResult.socialSecurity || 0) - foreignWithholding - typePersonalDeductions).toFixed(2)
    };

    // Add country-specific tax fields
    if (additionalTaxes && Object.keys(additionalTaxes).length > 0) {
      Object.assign(typeRecord, residency.formatAdditionalTaxesForOutput(additionalTaxes));
    }
    ...
    totalAdditionalTaxes += additionalTaxAmount;
  ...
  // Get annual additional taxes
  const annualAdditionalTaxes = residency.getAnnualAdditionalTaxes ? 
    residency.getAnnualAdditionalTaxes(yearRecords, specialRegimeStatus, year) : {};
  ...
  const annual = {
    ...
    NetIncome: (totalGross - totalTax - totalSocialSecurity - totalForeignWithholding - totalPersonalDeductions - (annualAdditionalTaxes.amount || totalAdditionalTaxes)).toFixed(2)
  };

  // Add country-specific tax fields to annual summary
  if (annualAdditionalTaxes && Object.keys(annualAdditionalTaxes).length > 0) {
    Object.assign(annual, residency.formatAdditionalTaxesForOutput(annualAdditionalTaxes));
  }
