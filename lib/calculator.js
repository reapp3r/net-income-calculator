 ...
 */
function calculateNetIncome(data, options = {}) {
  ...
  for (const year of sortedYears) {
    ...
    for (const period of periods) {
      ...
      for (const record of periodIncomeRecords) {
        ...
        const fullResult = {
          ...
          SpecialRegimeStatus: period.specialRegimeStatus ? 
            (period.specialRegimeStatus.regime || 'Standard') : 'Standard'
        };
      ...
      const annualResult = calculateAnnualSummaryForPeriod(period, periodMonthlyResults, residencyImpl);
...
function getIncomeForPeriod(period, incomeRecords) {
  ...
  return allRecords.filter(record => {
    ...
    if (period.startMonth && period.startDay) {
      ...
      if (record.month === period.startMonth && record.day < period.startDay) return false;
    }
    
    // Check end date (inclusive)
    if (period.endMonth && period.endDay) {
      if (record.month > period.endMonth) return false;
      if (record.month === period.endMonth && record.day > period.endDay) return false;
    }
 ...
 * @param {Object} residencyImpl - Residency implementation
 * @returns {Object} Annual summary
 */
function calculateAnnualSummaryForPeriod(period, monthlyResults, residencyImpl) {
  let totalGross = 0;
  let totalTax = 0;
  ...
  for (const result of monthlyResults) {
    ...
    const grossIncome = result.GrossIncome !== undefined ? result.GrossIncome :
                       result.amount !== undefined ? result.amount : 0;
    const taxAmount = result.TaxAmount !== undefined ? result.TaxAmount :
                     result.taxAmount !== undefined ? result.taxAmount : 0;
    const socialSecurity = result.SocialSecurity !== undefined ? result.SocialSecurity :
                         result.socialSecurity !== undefined ? result.socialSecurity : 0;
    const personalDeductions = result.PersonalDeductions !== undefined ? result.PersonalDeductions :
                              result.personalDeductions !== undefined ? result.personalDeductions : 0;
    const solidarityTax = result.SolidarityTax !== undefined ? result.SolidarityTax :
                        result.solidarityTax !== undefined ? result.solidarityTax : 0;
    const net = result.NetIncome !== undefined ? result.NetIncome :
              result.netIncome !== undefined ? result.netIncome : 0;
...
function calculateAnnualTypeResultsForPeriod(period, monthlyResults) {
  ...
  for (const result of monthlyResults) {
    ...
    // Handle both legacy (PascalCase) and new (camelCase) field names
    const grossIncome = result.GrossIncome !== undefined ? result.GrossIncome : result.amount || 0;
    const taxableIncome = result.TaxableIncome !== undefined ? result.TaxableIncome : result.taxableIncome || 0;
    const taxAmount = result.TaxAmount !== undefined ? result.TaxAmount : result.taxAmount || 0;
    const socialSecurity = result.SocialSecurity !== undefined ? result.SocialSecurity : result.socialSecurity || 0;
    const net = result.NetIncome !== undefined ? result.NetIncome : result.netIncome || 0;
